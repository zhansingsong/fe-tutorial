...
为了避免css样式被解析为XML标签，需要将样式写在 `CDATA` 中
```html
<svg>
  <style>
    <![CDATA[
...
Canvas API是Web平台的一个很好的补充，它具有与SVG类似的浏览器支持。与SVG的主要差异在于Canvas不是基于矢量的，而是基于像素的。
...
SVG API 提供了 line, rect(angle), circle, ellipse, polyline, polygon, text, and path 等基本图形
- line
- rect
- circle
- ellipse
